...
load('FieldII_FocTx.mat'); % Point Targets and Lesions Phantom
...
tic; img_retroTx = zeros(numel(r_img), numel(txAptPos(:,1)), numel(tx_origin_x));
...
for kk = 1:numel(tx_origin_x)
    [focData, includePoint] = ...
        focus_retroTx(time, rcvdata(:,:,kk), foc_pts, rxAptPos, ...
...
numTxForReconPt = sum(includePtInRecon, 3); toc;
