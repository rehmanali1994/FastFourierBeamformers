function [migSIG,param] = lumig(SIG,param)

%LUMIG   f-k migration for plane wave imaging (Lu's Algorithm)
%   MIGSIG = LUMIG(SIG,PARAM) performs a f-k migration of the signals
...
%
%   IMPORTANT NOTE: LUMIG does not use the transmit time delays as input
...
for k = 1:Nframes
    ...
    %-- Stolt's Mapping for Lu's Method
    kz = 2*f/c; % Choose kz = 2*f/c (i.e. z = c*t/2);
    fkz = (c/2)*(kx.^2+kz.^2)./(kx*sinA+kz*cosA); fkz(1,1) = 0;
