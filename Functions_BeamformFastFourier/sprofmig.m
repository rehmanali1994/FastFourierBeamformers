function [migSIG,param] = sprofmig(SIG,param)

%SPROFMIG   shot-profile Stolt's migration
%   MIGSIG = SPROFMIG(SIG,PARAM) performs a f-k migration of the signals
...
%
%   Hunter et al., The Wavenumber Algorithm for Full-Matrix 
%   Imaging Using an Ultrasonic Array. 
%
%   Ali, Fourier-based Synthetic-aperture Imaging for
%   Arbitrary Transmissions by Cross-correlation of 
%   Transmitted and Received Wave-fields
%   
...
%- x-direction
factor = 2.5; % 1.5 is too small, shot-profile needs more zero padding
...
k0 = f0/c; kz = 2*k0; 
kx = (-nxFFT/2+1:nxFFT/2)/param.pitch/nxFFT;
startTimeShift = exp(-1i*2*pi*k0*c*t0); %- Start Time Shift

%-- Pulsed-Wave Frequency Response on Transmit and Receive
txdata_f = zeros(numel(f0), nx, nTx);
rxdata_f = startTimeShift.*fftshift(fft(SIG,ntFFT),1);
for tx_idx = 1:nTx 
    % Construct Transmit Responses for Each Element
    txdata_f(:,:,tx_idx) = ...
        param.apod(tx_idx,:).*exp(-1i*2*pi*f0*param.delays(tx_idx,:));
end

%-- Take Lateral Spatial FFTs
txdata_kx_f = fftshift(fft(txdata_f, nxFFT, 2), 2);
rxdata_kx_f = fftshift(fft(rxdata_f, nxFFT, 2), 2);

%-- Shot-Profile Stolt's Migration
migSIG = zeros(numel(kz), 2*nxFFT-1);
for ktx_idx = 1:nxFFT
    % Extract Single Slice (in Ktx) of Tx Wavefield 
    conj_txdata_ktx_f = squeeze(conj(txdata_kx_f(:,ktx_idx,:)));
    ktx = kx(ktx_idx); ktz = sign(k0).*sqrt(k0.^2 - ktx^2); % Create ktx and ktz
    % Alternative Implementation of Stolt's Mapping
    migSIG_ktx = zeros(numel(kz), nxFFT);
    for krx_idx = 1:nxFFT
        % Extract Single Slice (in Krx) of Rx Wavefield 
        rxdata_krx_f = squeeze(rxdata_kx_f(:,krx_idx,:));
        % Create krx and krz
        krx = kx(krx_idx); krz = sign(k0).*sqrt(k0.^2 - krx^2);
        % Compute validity_region, directivity, and k to interpolate over
        validity_region_tx_rx = (k0 > 0) & (k0.^2 > ktx^2) & (k0.^2 > krx^2);
        directivity = krz(validity_region_tx_rx) .* ktz(validity_region_tx_rx); 
        kinterp = real(sqrt(4*ktx*krx*((krx-ktx)^2+kz.^2)+((krx-ktx)^2+kz.^2).^2))./(2*kz);
        % Extract Values over Validity Region
        migSIG_ktx_krx = sum(rxdata_krx_f(validity_region_tx_rx,:) .* ...
            conj_txdata_ktx_f(validity_region_tx_rx,:),2) .* directivity;
        % Interpolate over Valid Kz
        migSIG_ktx_krx = interp1(k0(validity_region_tx_rx), ...
            migSIG_ktx_krx, kinterp, 'spline', 0);
        % Exclusion Zone: Remove Incorrectly Mapped Signals
        migSIG_ktx_krx( ((krx^2+kz.^2)<=(ktx^2)) | ...
            ((krx^2-kz.^2)>=(ktx^2)) | (kz == 0)) = 0;        
        migSIG_ktx(:,krx_idx) = migSIG_ktx_krx;
    end
    % Stolt Map this Slice and Accumulate
    migSIG(:, nxFFT+(1:nxFFT)-ktx_idx) = ...
        migSIG(:, nxFFT+(1:nxFFT)-ktx_idx) + migSIG_ktx;
end
