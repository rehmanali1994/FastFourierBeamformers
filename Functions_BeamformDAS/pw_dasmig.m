function [migSIG,param] = pw_dasmig(SIG,param)

%PW_DASMIG   delay-and-sum beamforming for plane-wave imaging
%   MIGSIG = PW_DASMIG(SIG,PARAM) beamforms (delay-and-sum) the signals
%   stored in the array SIG. MIGSIG contains the beamformed signals. PARAM
%   is a structure that contains all the parameter values required for the
%   beamformation (see below for more details).
%
%   [MIGSIG,PARAM] = DASMIG(SIG,PARAM) also returns the complete list of
%   parameters:
%   PARAM.x and PARAM.z correspond to the coordinates at which the
%   beamformed signals are returned. The x-axis is parallel to the 
%   transducer and pointing from element #1 to element #N (PARAM.x = 0 at 
%   the CENTER of the transducer). The z-axis is PERPENDICULAR to the 
%   transducer and pointing downward (PARAM.z = 0 at the level of the 
%   transducer).
%
%   Important details on DASMIG:
%   --------------------------
...
%   8) PARAM.fnumber: F-number for receive aperture growth (default = 1);
%
%   IMPORTANT NOTE: DASMIG does not use the transmit time delays as input
...
%   -- Rehman Ali -- Adapted from fkmig 2023/02
...
assert(fnumber>0,'PARAM.fnumber must be greater than 0.')
%----- end of Input parameters -----

SIG = double(SIG);

%-- Time axis and Temporal shift
t0 = param.t0;
...
%-- Time for scattering from image point to reach receivers
migSIG = zeros(nt,nx);
rx_times = zeros(nt, nx, nx);
rx_win = zeros(nt, nx, nx); %- aperture growth
for rx = 1:nx
    rx_times(:,:,rx) = sqrt((x-param.x(rx)).^2+z.^2)/c;
    rx_win(:,:,rx) = (z>fnumber*abs(x-param.x(rx)));
end
...
%-- Loop over plane wave angles
for k = 1:Nframes

    %-- Plane wave data for this angle
    SIGk = SIG(:,:,k);
    sinA = sin(param.TXangle(k));
    cosA = cos(param.TXangle(k));
    
    %-- Time for plane wave to reach imaging point
    if param.TXangle(k) > 0
        tx_times = (cosA*z + sinA*(x-param.x(1)))/c;
    else
        tx_times = (cosA*z + sinA*(x-param.x(end)))/c;
    end
    
    %-- Interpolation in time for delay-and-sum
    migSIGk = zeros(nt,nx);
    for xim = 1:nx %- Loop over x coordinate in image
        times2way = repmat(tx_times(:,xim),[1,nx])+squeeze(rx_times(:,xim,:))-t0(k);
        apertGrowthWin = squeeze(rx_win(:,xim,:)); %- Aperture growth window
        if param.p
            %- p-point sinc interpolation
            migSIGk(:,xim) = sum(apertGrowthWin .* ...
                interpSINC(1/param.fs,SIGk,times2way,param.p,([0 1]+wn)/2),2);
        else
            %- linear interpolation
            migSIGk(:,xim) = sum(apertGrowthWin .* ...
                interpLIN(1/param.fs,SIGk,times2way,([0 1]+wn)/2),2);
        end
    end
    
    %-- Compounding
    migSIG = ((k-1)*migSIG + migSIGk)/k;
